...
def train():
    ...
    parser.add_argument("--model_checkpoint", type=str, default="./model/trained/model_epoch40_50w", help="Path or URL of the model")
    ...
    parser.add_argument("--train_path", type=str, default="./dataset/LCCC-base_train.json",
    ...
    print('当前GPU: ', torch.cuda.get_device_name(0), end='\n')
    ...
    # 使用Bert的分词器构建tokenizer
    tokenizer = BertTokenizer.from_pretrained(args.model_checkpoint, do_lower_case=True)

    # 加载训练数据与验证数据
    # 获取到加载到训练器的数据集
    train_loader, val_loader, train_sampler, valid_sampler = build_dist_loaders(args, tokenizer, logger)

    # model的初始化
    model = GPT2LMHeadModel.from_pretrained(args.model_checkpoint)

    # 设置需要微淘的层、只设置最后3层为可训练
    model.transformer.h[:9].trainable = False
    model.transformer.h[9:].trainable = True
    # model = GPT2LMHeadModel.from_pretrained('./model/bert/gpt2-13088.bin')
    ...
    def update(engine, batch):
        # 获得每一个bitch的input_ids, token_type_ids, attention_mask
        ...
        # 每args.gradient_accumulation_steps步更新优化器,并清零梯度
    ...
    if args.local_rank in [-1, 0]:
        ...
        checkpoint_handler = ModelCheckpoint('custom/', 'checkpoint', save_interval=1, n_saved=3)
        ...
        # torch.save(args, './' + '/model_training_args.bin')
        # getattr(model, 'module', model).config.to_json_file(os.path.join('./', CONFIG_NAME))
        # tokenizer.save_vocabulary('./')

    # Run the training
    trainer.run(train_loader, max_epochs=args.n_epochs)
    # evaluator.run(val_loader, max_epochs=args.n_epochs)

    # 保存训练好模型的tokenizer
    # tokenizer_path = './model/trained/bert/tokenizer.json'
    # tokenizer.save_vocabulary(tokenizer_path)

    # 保存训练好的模型
    model.save_pretrained('./model/trained/chinese_gpt2_tune/')
